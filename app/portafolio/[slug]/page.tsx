...
import { getStrapiData, capitalizeSlug } from '@/app/lib/strapi';
...
export async function generateMetadata({params}: {params: { slug: string } | Promise<{ slug: string }>;}) {
  ...
  return {
    title: `${school} | Filmo`,
    description: `Cobertura audiovisual realizada para ${school}.`,
    alternates: {
      canonical: `/portafolio/${slug}`,
    },
    openGraph: {
      title: `${school} | Filmo`,
      description:
...
export default async function SchoolPage({
  ...
  const schemaSchoolData = {
    "@context": "https://schema.org",
    "@type": "EducationalOrganization",
    "name": schoolName,
    "url": `https://filmostudio.com/portafolio/${slug}`,
    "description": `Cobertura audiovisual realizada para ${schoolName}.`,
  };
  ...
  return (
    <>
      <script
        ...
        dangerouslySetInnerHTML={{ __html: JSON.stringify(schemaSchoolData) }}
      />

      <Navbar />

      <main className="bg-filmo-black-100 flex flex-col">
        <section className="flex h-[800px] max-md:h-[550px] w-full overflow-hidden">
